...
module("ygmutils", package.seeall)
...
function redis_cluster(host)
    local client = Redis:new()
    if not client then
        return nil
    end

    client:set_timeout(3000) -- 2 seconds
    local ok, err = client:connect(host, redis_port)
    if not ok then
        return nil
    end
